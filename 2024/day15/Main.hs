...
findStart m = head $ filter (\(x, y) -> getElem y x m == '@') [(x, y) | x <- [1 .. ncols m], y <- [1 .. nrows m]]
...
moveWide m p@(x, y) d =
  let p'@(x', y') = nextPos d p
   in case (getElem y x m, isUpDown d) of
        ('.', _) -> Just m
        ('#', _) -> Nothing
        (c, False) -> case moveWide m p' d of
          Just m' ->
            let m'' = setElem (getElem y' x' m') (y, x) m'
             in Just $ setElem c (y', x') m''
          Nothing -> Nothing
        (c, True) ->
          moveWide m p' d >>= \m' ->
...
moveRobotWide m p d = case moveWide m p d of
  Nothing -> (m, p)
  Just m' -> (m', nextPos d p)

getMovePos :: Matrix Char -> (Int, Int) -> Char -> Maybe (Int, Int)
getMovePos m p@(x, y) dir = case getElem y x m of
  '.' -> Just p
  '#' -> Nothing
...
moveRobot m p@(x, y) dir = case getMovePos m (nextPos dir p) dir of
  Nothing -> (m, p)
  Just (x', y') ->
...
calcScore m = sum [factor (getElem y x m) * (100 * (y - 1) + (x - 1)) | x <- [1 .. ncols m], y <- [1 .. nrows m]]
  where
    factor 'O' = 1
    factor '[' = 1
    factor _ = 0
...
solve1 s =
  let (m, moves) = parseInput id s
   in calcScore . fst $ foldl (\(m', p) dir -> moveRobot m' p dir) (m, findStart m) moves

solve2 :: String -> Int
solve2 s =
  let (m, moves) = parseInput widenMat s
   in calcScore . fst $ foldl (\(m', p) dir -> moveRobotWide m' p dir) (m, findStart m) moves
